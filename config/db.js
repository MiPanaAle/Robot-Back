...
const validateConfig = () => {
  ...
  console.log(`   - Encriptación: ${connection.options.encrypt ? 'Habilitada' : 'Deshabilitada'}`);
};
...
const poolConnect = pool.connect()
  ...
  .catch(err => {
    ...
    if (err.code === 'ECONNREFUSED') {
      console.error('💡 Sugerencia: Verifica que SQL Server esté ejecutándose y que el puerto esté abierto');
    } else if (err.code === 'ENOTFOUND') {
      console.error('💡 Sugerencia: Verifica la dirección del servidor en DB_SERVER');
    } else if (err.number === 18456) {
      console.error('💡 Sugerencia: Verifica las credenciales de usuario y contraseña');
    } else if (err.number === 4060) {
      console.error('💡 Sugerencia: Verifica que la base de datos especificada existe');
    }
